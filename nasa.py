...
SCREEN_SIZE = (1041, 601) #resolution of the game
...
gravity = int(input("Gravitational field strength (N/Kg): "))
...
class Crashman(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        self.contact = False
        self.jump = False
        self.image = pygame.image.load('actions/right.png').convert()
        self.rect = self.image.get_rect()
        self.run_left = ['actions/right.png']
        self.run_right = ['actions/left.png']
    ...
    def update(self, up, down, left, right):
        ...
        if down:
            if self.contact and self.direction == "right":
                self.image = pygame.image.load('actions/right.png').convert_alpha()
            if self.contact and self.direction == "left":
                self.image = pygame.image.load('actions/left.png').convert_alpha()

        if up and self.direction == "right":
            self.image = pygame.image.load('actions/rfire.png').convert_alpha()

        if up and self.direction == "left":
            self.image = pygame.image.load('actions/lfire.png').convert_alpha()
